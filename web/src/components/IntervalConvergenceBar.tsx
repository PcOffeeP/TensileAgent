...
export default function IntervalConvergenceBar({ trace, fallbackResult }: IntervalConvergenceBarProps) {
  ...
  if (!duration) {
    return (
      <div className="glass-panel p-4 rounded-xl border border-slate-200 shadow-sm flex items-center justify-between">
        <span className="text-sm font-medium text-slate-700">候选区间收敛过程</span>
        <span className="text-xs text-slate-400">等待获取视频总时长...</span>
      </div>
    );
  }
  ...
  return (
    <div className="glass-panel p-5 rounded-xl border border-slate-200 shadow-sm flex flex-col gap-4 bg-white/80">
      ...
      <div className="relative h-12 bg-slate-100 rounded-lg overflow-hidden border border-slate-200/50">
        {/* Background scale marks could go here */}
        ...
        {currentCandidate && !finalRange && (
          <div 
            className="absolute top-1 bottom-1 bg-amber-400/30 border-2 border-amber-500 rounded-md shadow-sm transition-all duration-500"
            style={{ 
        ...
        {finalRange && (
          <div 
            className="absolute top-0 bottom-0 bg-rose-500 border-2 border-rose-600 rounded-md shadow-md z-10 animate-pulse"
            style={{ 
      ...
      <div className="flex items-center gap-4 text-xs text-slate-500 justify-end">
        <div className="flex items-center gap-1.5">
          <div className="w-3 h-3 bg-[#002FA7]/10 border border-[#002FA7]/20 rounded-sm"></div>
          <span>历史采样</span>
        </div>
        <div className="flex items-center gap-1.5">
          <div className="w-3 h-3 bg-amber-400/30 border-2 border-amber-500 rounded-sm"></div>
          <span>当前候选</span>
        </div>
        <div className="flex items-center gap-1.5">
          <div className="w-3 h-3 bg-rose-500 border-2 border-rose-600 rounded-sm"></div>
          <span>最终断裂</span>
        </div>
      </div>
    </div>
