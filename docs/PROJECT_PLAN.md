...
Agent 最终业务结论包含 `has_fracture`、原视频 `time_range`、`type`、`location`、分项 `confidence` 和按需 `visual_evidence`。局部 `fracture_between` 只保存在对应轮次证据中，必须绑定 clip、实际帧表和映射元数据，不能直接充当最终原视频区间。`confidence` 至少区分决策、定位、分类和整体可靠度；未完成按源视频隔离的校准前不得伪装成概率。`visual_evidence` 默认内部可追踪、按需面向用户展示；当前基线通过第二次固定视觉调用在同一批实际输入帧上生成语义摘要，不允许 TensileAgent 根据四字段凭空编写视觉现象。
