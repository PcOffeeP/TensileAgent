...
TensileAgent 是材料拉伸试验视频的本地 Agent 分析系统。它不再维护模型微调、数据治理、训练样本构建、LLaMA-Factory 配置或训练机工作流；这些能力由外部训练/推理项目提供。本仓库只负责把一个已部署的 OpenAI-compatible 视觉推理服务接入到可解释、可追踪、可复现的多轮 Agent 决策链路中。

项目的核心目标是：对单个拉伸试验视频执行多轮区间检查，调用外部视觉模型判断局部片段，再由确定性状态机验证证据、收敛候选区间并输出 `fracture`、`no_fracture` 或 `unrecognized` 三种公共结论。
